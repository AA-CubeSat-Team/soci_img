...
void testSetExposure(){
  Serial.println("--beginning testSetExposure--"); //Use Serial.print to help you debug!
...
void takePicture(byte value) {
  for (int trial = 0; trial < 5; trial++) {
    ...
    if(responseBytes[0] != ACK ||
     ...
     responseBytes[2] != value) {
        Serial.print("\nFAIL: Did not give proper response when Value (takePicture) = "); Serial.println(value);
        Serial.print("Received: "); 
        for(int j = 0; j < MAX_RESPONSE_BYTES; j++) {
           Serial.print("0x"); Serial.print(responseBytes[j], HEX); Serial.print(" ");
        }
